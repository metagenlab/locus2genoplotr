...
class Subrecord:
    def __init__(self, gbk_record, out_prefix, basename=False):
        ...
        self.faa_record = self.write_coding_features_to_fasta(
            gbk_record, self.faa_file_path
        )
            ...
            for feature in record.features:
                if feature.type == "CDS" and "translation" in feature.qualifiers:
                    ...
                    if "locus_tag" in feature.qualifiers:
                        ...
                        logging.warning(
                            f"Missing locus_tag for protein_id {feature.qualifiers['protein_id'][0]}"
                        )
                    else:
                        logging.warning(
                            f"Missing locus_tag and protein_id for feature at location {feature.location}"
                        )
                    ...
                    logging.warning(
                        f"Missing translation for feature at location {feature.location}"
                    )
...
class GenomeComp:
    def __init__(
        ...
        if os.path.exists(self.output_folder):
            if not force_data_dir:
                raise IOError(
                    "Output folder already present in the working directory, remove it or use -f to force rewrite"
                )
            else:
        ...
        self.ref_subset = self.get_sequence_subset(
            self.reference_records["records"],
            query_locus,
            upstream_bp,
            downstream_bp,
        ...
        self.ref_locus_record = SeqRecord.SeqRecord(
            self.ref_subset["seq"],
            id=self.query_locus,
        ...
        self.ref_subrecord = Subrecord(
            self.ref_subset["subset_record"],
        ...
        self.target_full_records = [
            Subrecord(
                target["records"],
                f"{self.output_folder}/{target['basename']}.full",
                target["basename"],
            )
            for target in self.target_records_list
        ]

        logging.info("Extracting bbh from targets")
        self.target_BBH_locus = [
            self.blast_best_hit(self.ref_locus_faa, i.faa_file_path)
            for i in self.target_full_records
        ]
        print(self.target_BBH_locus)
        logging.info("Extracting subrecords targets")
        print(self.target_BBH_locus)
        print(self.target_records_list)
        self.target_subrecords = [
            self.get_sequence_subset(
                seqrecords["records"],
                target_id["subject_id"],
                upstream_bp,
                downstream_bp,
                basename=seqrecords["basename"],
                flip_record=flip_records,
            )
            for target_id, seqrecords in zip(
                self.target_BBH_locus, self.target_records_list
            )
        ]

        self.target_faa_subrecords = [
            Subrecord(
                target_sub["subset_record"],
                f"{self.output_folder}/{target_sub['basename']}.sub",
                basename=target_sub["basename"],
            )
            for target_sub in self.target_subrecords
        ]

        logging.info("Perform blast comparisons of the reference and targets")
        print([i.basename for i in self.target_faa_subrecords])
        self.blast_comparaisons = self.record_list2blast(
            [self.ref_subset["subset_record"]]
            + [i["subset_record"] for i in self.target_subrecords],
            min_identity,
        )

        # create a combined fasta with all sequences
        records = chain.from_iterable(
            [target_sub.faa_record for target_sub in self.target_faa_subrecords]
            + [self.ref_subrecord.faa_record]
        )
        ...
        self.clusters, self.locus2cluster = self.cluster_protein_sequences(
            f"{self.output_folder}/combined_sub.faa"
        )
        self.cluster2annot = {}
    ...
    def get_sequence_subset(
        ...
        for record in seqrecords:
            for f in record.features:
                if (
                    ...
                    and target_id == f.qualifiers["protein_id"][0]
                ):
                    feature = f
                    match_record = record
                    ...
                    break
                if (
                    "locus_tag" in f.qualifiers
                    and target_id == f.qualifiers["locus_tag"][0]
                ):
                    feature = f
                    match_record = record
                    seq = feature.extract(record.seq).translate()
        ...
        if flip_record:
            if force_flip:
                ...
                subset_record = match_record[subset_start:subset_end]
                subset_record = subset_record.reverse_complement(
                    id=subset_record.id,
    ...
    def add_gbk_track(
        ...
        track = gv.add_feature_track(label, len(gbk.seq))
        ...
        for feature in gbk.features:
            if feature.type == "CDS":
                ...
                track.add_feature(
                    ...
                    hatch=hatch,
                    text_kws=dict(
                        rotation=45, hpos="left", vpos="top", color=labelcolor
                    ),
                )
    ...
    def parse_highlights(self, path):
        ...
        for n, row in df.iterrows():
            ...
            self.cluster2highlights[cluster] = {
                "color": row["color"],
                "label": row["label"],
            }
    ...
    def get_cluster_consensus_annot(self, records):
        ...
        for cluster in self.clusters:
            ...
            self.cluster2annot_consensus[cluster] = {
                "freq": cluster2n_genomes[cluster],
                "gene": gene_c,
                "product": product_c,
            }
    ...
    def compare_neigborhood(
        ...
        with open(f"{self.output_folder}/cluster_summary.tsv", "w") as f:
            ...
            for cluster in self.cluster2annot_consensus:
                f.write(
                    f"{cluster}\t{self.cluster2annot_consensus[cluster]['freq']}\t{self.cluster2annot_consensus[cluster]['gene']}\t{self.cluster2annot_consensus[cluster]['product']}\n"
                )

        # {"basename": basename, "records": records})
        with open(f"{self.output_folder}/cluster_comparaison.tsv", "w") as f:
            f.write(
                "genome_1\tgenome_2\tn_clusters_1\tn_clusters_2\tn_shared\tn_unique_1\tn_unique_2\tjaccard\n"
            )
            for pair in combinations(all_records, 2):
                ...
                f.write(
                    f"{genome_label_1}\t{genome_label_2}\t{len(clusters_1)}\t{len(clusters_2)}\t{len(shared)}\t{len(unique_1)}\t{len(unique_2)}\t{1 - jaccard}\n"
                )

    def cluster_protein_sequences(self, faa_file, min_identity=0.5, cluster_mode="0"):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ls", "--left_side_window", type=int, help="left siden window", default=15000
    )
    parser.add_argument(
        "-rs", "--right_side_window", type=int, help="right side window", default=15000
    )
    parser.add_argument(
        "-i", "--min_identity", type=int, help="minimum identity for blast", default=50
    )
    parser.add_argument(
        "-sl", "--show_labels", action="store_false", help="do not show show labels"
    )
    parser.add_argument(
        "-s",
        "--samtools_depth",
        default=False,
        help="add depth plot from samtool depth (only for the last query). Should match the chromosome/contig names of the gbk.",
    )
    parser.add_argument(
        "-x",
        "--tblastx",
        action="store_true",
        help="execute tblastx and not blasn (6 frame translations)",
    )
    ...
    parser.add_argument(
        ...
        "--force",
        action="store_true",
        help="Don't prompt before output folder removal",
    )
    ...
    if args.reference in targets:
        ...
        targets.pop(targets.index(args.reference))
    ...
    reg_highlights = {
        r".*IS[0-9]+": {"color": "#ff00e0", "label": None},
